...
export function UnifiedNapForm({
  mode,
  startEventId,
  initialStartTime,
  initialEndTime,
  ...
  return (
    <div className="space-y-6">
      <SleepEventFields
        startTimeLabel="Nap Started"
        endTimeLabel="Nap Ended"
        endTimePlaceholder="Tap to set end time..."
        endTimeHint="💡 Leave blank if still napping"
        startTime={startTime}
        ...
        inProgressText={
          mode === 'edit' && !endTime && initialStartTime
            ? `In progress for ${formatDuration(calculateDurationMinutes(initialStartTime, new Date().toISOString()))}`
            : undefined
        }
      />
        ...
        {mode === 'edit' && onDelete && (
          ...
          >
            {isDeleting ? (
              ...
              <><Trash2 className="size-4 mr-2" />Delete Nap</>
            )}
          </Button>
        )}
        ...
        <div className="flex gap-2">
          ...
          >
            {isSaving ? (
              ...
              mode === 'edit' ? 'Save Changes' : 'Save Nap'
            )}
          </Button>
        </div>
      </div>
