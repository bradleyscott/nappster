...
describe('useTodaySleepState', () => {
  ...
  it('returns awaiting_morning_wake when no events', () => {
    const { result } = renderHook(() => useTodaySleepState([], 'UTC'))
  ...
  it('returns daytime_awake after wake', () => {
    ...
    const { result } = renderHook(() => useTodaySleepState(events, 'UTC'))
  ...
  it('respects timezone for today bounds', () => {
    // 12:00 UTC = 7:00 EST. Bedtime at 4:00 UTC = 11pm EST the previous day
    const events = [makeEvent({ event_type: 'bedtime', event_time: '2024-01-15T04:00:00Z' })]
    const { result } = renderHook(() => useTodaySleepState(events, 'America/New_York'))
    expect(result.current).toBe('awaiting_morning_wake')
  })
})
